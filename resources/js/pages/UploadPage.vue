<template>
    <section class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-3xl border border-white/10 bg-slate-900/60 p-8 shadow-xl">
            <h1 class="text-3xl font-semibold">Datei hochladen</h1>
            ...
            <div class="mt-8 flex flex-col gap-4">
                <label
                    ...
                    <span class="text-sm uppercase tracking-widest text-slate-400">Dateien auswählen</span>
                    <span class="text-lg font-medium">
                        {{ files.length ? `${files.length} Dateien ausgewählt` : 'Drag & Drop oder klick hier' }}
                    </span>
                    <span class="text-xs text-slate-500">Maximal 1 GB pro Datei (je nach Server-Config)</span>
                </label>

                <div v-if="files.length" class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-slate-400">Ausgewählte Dateien</p>
                            <p class="text-lg font-medium">{{ files.length }} Datei(en)</p>
                        </div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4 md:col-span-2">
                        <label class="text-xs uppercase tracking-widest text-slate-400">E-Mail (Pflicht)</label>
                        <input v-model.trim="uploaderEmail" type="email" required
                            placeholder="z. B. info@stratton.cologne"
                    ...
                    <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                        <label class="text-xs uppercase tracking-widest text-slate-400">Ablaufzeit (Tage)</label>
                        <input v-model.number="expiresInDays" type="number" min="1" max="30" required
                            placeholder="z. B. 7"
                            class="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none" />
                    </div>
                    <div class="rounded-2xl border border-white/10 bg-slate-950/40 p-4">
                        <label class="text-xs uppercase tracking-widest text-slate-400">Max. Downloads</label>
                        <input v-model.number="maxDownloads" type="number" min="1" max="1000" placeholder="z. B. 50"
                            class="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/60 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none" />
                    </div>
                </div>
                ...
                <button
                    ...
                    :disabled="!files.length || isUploading || !uploaderEmail" @click="upload">
                    Upload starten
                </button>
            </div>
        </div>
        ...
        <aside class="space-y-6">
            <div class="rounded-3xl border border-white/10 bg-slate-900/60 p-6">
                <h2 class="text-xl font-semibold">So funktioniert's</h2>
                <ul class="mt-4 space-y-3 text-sm text-slate-400">
                    ...
                    <li class="flex items-start gap-3">
                        ...
                        Share-Link kopieren und teilen.
                    </li>
                    <li class="flex items-start gap-3">
                        <span class="mt-1 h-2 w-2 rounded-full bg-emerald-500"></span>
                        Empfänger lädt die Datei sicher herunter.
                    </li>
                </ul>
            </div>

            <div class="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-950/80 p-6">
                <h2 class="text-xl font-semibold">Mehr Kontrolle</h2>
